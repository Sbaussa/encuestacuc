...
import { Plus, Trash2, GripVertical, ChevronLeft, Save, Loader2, AlertCircle } from "lucide-react";
...
export default function CreateSurveyPage() {
  ...
  const validate = () => {
    ...
    if (questions.length === 0) errs.push("Agrega al menos una pregunta");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors(["Error al guardar la encuesta. Inténtalo de nuevo."]);
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 pb-32">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Nueva encuesta</h1>
            <p className="text-slate-500 text-sm">Completa los campos y agrega preguntas</p>
          </div>
        </div>
        ...
        {errors.length > 0 && (
          <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-xl">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-medium text-red-600">Corrige los siguientes errores:</p>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-red-600 focus:bg-white transition-colors text-sm resize-none"
              />
            </div>
          </div>
        </div>
          ...
          {questions.map((question, qIdx) => (
            <div key={question.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  <p className="text-xs font-medium text-slate-500 mb-2.5">Opciones de respuesta</p>
                  ...
                  <button
                    ...
                    className="mt-2.5 flex items-center gap-1.5 text-red-600 hover:text-red-600 text-sm font-medium transition-colors"
                  >
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-md border-t border-slate-100">
        <div className="max-w-2xl mx-auto">
          <button
            ...
            disabled={saving}
            className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-semibold bg-gradient-to-r from-red-600 to-red-700 text-white shadow-lg shadow-red-600 hover:shadow-red-600 transition-all active:scale-98 disabled:opacity-50"
          >
